<template>
	<div class="w-full bg-white sticky top-0 z-[110]">
		<div
			...
			:class="`${!isIndex || isMobileScreen ? 'custom-size sticky' : 'py-[15px]'}`"
		>
			<div class="flex md:basis-1/4 relative">
				...
				<a :href="baseInfo['web_host']" target="_blank" class="header-text header-text-initial">
					{{ baseInfo.app_name }}
				</a>
			</div>
			...
			<div class="header-right-container flex-grow pr-[20px]">
				<div
					v-for="(rightItem, i) in headerConfig.right.filter((element) => element.is_show)"
				...
				>
					...
					>
						...
						<div
							...
							v-if="rightItem?.group?.children && rightItem?.group?.children.length > 0"
					...
					>
						<Icon v-if="rightItem?.icon" :name="rightItem?.icon" size="24"></Icon>
					</div>
				</div>
			</div>
		</div>
...
import { CONFIG_KEY_BASE, CONFIG_KEY_HERDER, getSiteConfigItem } from '~/stores/config'

const baseInfo = await getSiteConfigItem(CONFIG_KEY_BASE)
const headerConfig = await getSiteConfigItem(CONFIG_KEY_HERDER)
