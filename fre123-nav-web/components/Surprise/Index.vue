<template>
	<ClientOnly v-if="surpriseList">
		<!-- 右下角展示 -->
		<div
			class="fre123-surprise"
			v-for="surprise in surpriseList.filter(
...
import { CONFIG_BUSINESS_KEY_SURPRISE, getBusinessConfig } from '~/stores/config'
...
const hideSurprise = (s: ISurpriseItem) => {
	...
	surpriseList.value = surpriseList.value.filter((surprise) => surprise.title != s.title)
}
...
const getSurpriseList = async () => {
	...
	configData.forEach((formatItem: ISurpriseItem) => {
		...
		// 判断广告是否在有效期内
		if (formatItem.end_ts < curTime || formatItem.start_ts > curTime || !formatItem.is_show) {
			return
		}
	...
	console.log(`surpriseList `, surpriseList.value)
...
.fre123-surprise {
	...
	background: linear-gradient(110.17deg, #ffe3d4 -3.58%, #fffbd7 58.23%, #fbf2ff 103.91%);
}
</style>
