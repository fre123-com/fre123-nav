<template>
  <div class="table-box-children">
    <el-tabs
      v-model="activeAvertTab"
      tab-position="left"
      class="demo-tabs custom-tabs"
      @tab-change="tabChange()"
    >
      <el-tab-pane
        v-for="tab in advertBox"
        :key="tab?.name"
        :label="tab.label"
        :name="tab?.name"
    ...
    <MetaTable
      :tableData="currentAvertData ?? []"
      :propList="HeaderAdvertChildPropList"
      @addData="openAddChildDialog"
      @deleteData="deleteChild"
      @editData="openEditChildDialog"
      :isDraggable="true"
      :isSetTop="isSetTop"
      rowKey="id"
      :key="advertTableKey"
      ...
      @confirm-drag="updateAdvertChildDrag"
    >
...
import { advertChildFields } from '@/views/website/config/formConfig'
...
const addNewAdvertChild = async () => {
  ...
  updatedHeader.right[currentGroupIndex.value].group.children.push(newChildItem)
...
const deleteChild = (child: { name: string; url: string }) => {
