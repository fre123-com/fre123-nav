  ...
  <div class="content-five">
    <div class="pendant-config">
      <el-form-item label="是否展示" style="margin-left: 10px">
        <el-switch
          v-model="pendant.is_show"
          @change="handlePendentSwitch"
          :active-value="1"
          :inactive-value="0"
        ></el-switch>
      </el-form-item>
    ...
    <MetaTable
      :tableData="pendant?.list ?? []"
      :propList="PendantPropList"
      @addData="pendantDialogControl"
      @deleteData="removeItem"
      @editData="onEditPendant"
      :isDraggable="true"
      :isSetTop="isSetTop"
      rowKey="id"
      :key="pendantKey"
      ...
      >
        ...
        <el-form-item label="Hover类型">
          <el-select v-model="typeSelect" @change="handleTypeChange">
            <el-option
              v-for="option in getPendantTypeOptions()"
              :key="option.value"
              :label="option.label"
...
const fetchData = async () => {
  const data = await WebsiteStore().getWebsiteConfig<IPendantData>(
    TabType.Pendant
  )
  pendant.value = {
  ...
  formRef.value.validate(async (valid: boolean) => {
    if (valid) {
      if (currentPendantIndex.value !== null) {
        pendant.value.list[currentPendantIndex.value] = {
