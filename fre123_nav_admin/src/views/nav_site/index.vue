<template>
  <div>
    <el-tabs type="card" v-loading="!tableData || loading" style="display: block; width: 100%" @tab-click="TabClick">
      <el-tab-pane v-for="(group, groupIndex) in tableData" :label="group.group_name" :key="groupIndex">
        <template #default>
          <el-tabs v-model:activeName="tabName" tab-position="left" class="demo-tabs"
            style="display: block; width: 100%" @tab-click="handleTabClick">
            <div v-if="!group.tab_list || group.tab_list.length === 0">
              当前没有数据,请返回分类管理添加
            </div>
            <el-tab-pane v-for="(tab, tabIndex) in group.tab_list" :label="tab.tab_name" :key="tabIndex"
              :class="tab.tab_name">
              <MetaTable v-if="tabName === tab.tab_name" :propList="tableConfig.propList" :tableData="tab.details"
                @addData="newDialog(groupIndex, tabIndex)" @edit-data="showDialog($event, groupIndex, tabIndex)"
                @delete-data="deleteRow($event, groupIndex, tabIndex)" :isDraggable=true
                @confirm-drag="updateNewSort($event, groupIndex, tabIndex)" :key="metaTableKey">
                <template #icon="scope">
                  <el-image style="width: 50px; height: 50px" :src="scope.row.icon" :fit="scope.row.fit" />
                </template>
                <template #is_show="scope">
                  <el-switch v-model="scope.row.is_show" label="显示" :active-value="1" :inactive-value="0"
    ...
    <!-- 分割线---------下面是抽屉等等一系列东西------------------------------------------------------------------------ -->
    <SiteDialog :visible="currentRow.drawerVisible" :form-data="formData" :dialog-header="tShow" :edit-mode="bShow"
      @on-submit="onSubmit" @on-submit-new="newSubmit" @closed="currentRow.drawerVisible = false">
    </SiteDialog>
  </div>
</template>
...
const getIndex = (row: any, groupIndex: number, tabIndex: number) => {
  const rowIndex = tableData.value[groupIndex].tab_list[tabIndex].details.indexOf(row);
  return rowIndex;
};
...
const onSubmit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid) => {
    if (valid && editingIndex.value) {
      ...
      if (tableData.value) {
        const detail = tableData.value[groupIndex].tab_list[tabIndex].details[detailIndex];
        // 更新 detail 对象的属性
...
const deleteRow = (row: any, groupIndex: number, tabIndex: number) => {
  const detailIndex = getIndex(row, groupIndex, tabIndex);
  editingIndex.value = { groupIndex, tabIndex, detailIndex };
  tableData.value[groupIndex].tab_list[tabIndex].details.splice(
    detailIndex,
    1
  );
  updateHeader();
