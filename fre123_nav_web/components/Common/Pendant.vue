<template>
  <div
    ...
    id="navbar-pendant"
    class="z-50 fixed right-[10px] opacity-90 top-[50%]"
  >
    <ul>
      <li
        ...
        :class="`hover:text-${item?.icon_hover_color}`"
      >
        <component
          :is="item?.url ? 'a' : 'div'"
          :href="item?.url"
        ...
        >
          ...
          >
            ...
            <p class="text-[chocolate] text-[12px] whitespace-nowrap m-0">
              {{ item?.text }}
            </p>
          </div>
          <p
...
import { CONFIG_KEY_PENDANT, getSiteConfigItem } from '~/stores/config'

const pendantConfig = await getSiteConfigItem(CONFIG_KEY_PENDANT)
...
.navbar-icon-style {
  @apply w-[35px] h-[35px] mb-[5px] leading-[35px] rounded-[4px] bg-[#f8f9fa] text-center shadow-[0_2px_5px_rgba(0,0,0,0.3)] relative flex flex-row items-center justify-center;
}
#navbar-pendant > ul {
