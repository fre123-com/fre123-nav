...
const nativeAgents = { http: Rt, https: https };
...
const stream = defineProxyMiddleware(
  (req, socket, options, server, head, callback) => {
    ...
    const proxyReq = (isSSL.test(options.target.protocol) ? https : Rt).request(setupOutgoing(options.ssl || {}, options, req));
    if (server) {
...
class ProxyServer extends EventEmitter {
  ...
  listen(port, hostname) {
    ...
    this._server = this.options.ssl ? https.createServer(this.options.ssl, closure) : Rt.createServer(closure);
    if (this.options.ws) {
