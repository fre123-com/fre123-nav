    ...
    <div class="typeTable-top">
        <div class="data-show-number">
            <el-icon size="large" color="#409EFF" style="margin-right: 10px;">
                <List />
            </el-icon>类别数量：{{ props.tableList?.length }}
        </div>
        <div class="typeTable-top-refresh">
            <el-button circle size="large" type="info" plain :icon="Refresh"
                @click="friendshipLinkApi.refresh('刷新成功')"></el-button>
        </div>
    </div>

    <!-- 类别表格板块 -->
    <!-- 使用 MetaTable 组件 -->
    <MetaTable v-if="tableList?.length" @confirmDrag="updateNewSort" :isDraggable="true" :operateWidth="operateWidth" :key="metaTableKey" max-height="500"
        :is-loading="props.loading" :tableData="props.tableList" :propList="props.propList"
        @addData="dialogs[2].isVisible = true" @deleteData="deleteTypes" @editData="editGetKindOp"
        @setTop="upTop">
        <template #count="{ row }">
            <text class="countColor" @click="typeToUrl(row.name)" :class="{ 'disabled': row.count === 0 }">{{ row.count
                }}</text>
        </template>
        ...
        <template #status="{ row }">
            <el-switch v-model="row.status" :loading="switchLoading" :active-value="1" :inactive-value="0"
                @click="switchChange(row)"></el-switch>
        </template>
    </MetaTable>

    <!-- 使用 v-for 渲染多个 TypeDialog 组件 -->
    <TypeDialog v-for="dialog in dialogs" :key="dialog.id" :id="dialog.id" :rules="opRules" :title="dialog.title"
        :icon-type="dialog.iconType" :finishBtn="dialog.finishBtn" v-model="dialog.isVisible" :type-op="kindOp"
        :kind-finish-dialog="kindFinishDialog" :clean-dialog="cleanDialog" />

</template>

<script lang="ts" setup>
import TypeDialog from "@/views/friendship_link/components/TypeDialog.vue";
import { friendshipLinkApi, metaTableKey, tabName, tabActiveName, activeName, opForm, typeDataShow } from '@/views/friendship_link'
