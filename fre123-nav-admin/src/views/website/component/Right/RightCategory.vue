<template>
    <div class="content-three">
        <MetaTable :tableData="advertGroup" :propList="HeaderAdvertPropList" @addData="openAddAdvertDialog"
            @deleteData="deleteAdvert" @editData="openEditDialog" v-if="advertGroup?.length" :isDraggable="true"
            :isSetTop="isSetTop" rowKey="id" :key="advertGroupKey" :operateWidth="105" :is-loading="isLoading"
            ...
            <template #is_show="{ row }">
                <el-switch v-model="row.is_show" @change="saveAdvertGroupSwitchChange(row)" :active-value="1"
                    :inactive-value="0">
                </el-switch>
            </template>
        </MetaTable>
...
const updateAdvertDrag = async (list: any) => {
    ...
    const advertGroups = updatedHeader.right;
    const urlOrder = new Map<string, number>();
    list.forEach((item: { url: string; }, index: number) => {
        urlOrder.set(item.url, index);
    });
    advertGroups.sort((a, b) => {
        const indexA = urlOrder.get(a.group.url) ?? -1;
        const indexB = urlOrder.get(b.group.url) ?? -1;
    ...
    isLoading.value = true;
    await updateWebsite(requestBody.value);
    storedData.value = updatedHeader;
    advertKey.value++;
