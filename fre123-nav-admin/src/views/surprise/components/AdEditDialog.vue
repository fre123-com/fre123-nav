<template>
  <el-dialog :title="dialogTitle" width="40%" @close="cancelDialog">
    <el-form :model="editFormData" :rules="rules" ref="editFormRef" label-width="100px" class="form-style">
      <el-form-item label="广告标题" prop="title">
        <el-input v-model="editFormData.title"></el-input>
      </el-form-item>
      <el-form-item label="广告类型" prop="type">
        <el-select v-model="editFormData.type" @change="handleTypeChange">
          <el-option v-for="option in getSurpriseTypeOptions()" :key="option.value" :label="option.label"
            :value="parseInt(option.value, 10)"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item v-if="showDescription" label="文字描述" prop="description" style="margin-left: 30px">
        <el-input v-model="editFormData.description"></el-input>
      </el-form-item>
      <el-form-item v-if="showImageUrlInput" label="图片链接" prop="img_url" style="margin-left: 30px">
        <el-input v-model="editFormData.img_url" placeholder="请输入以http://或https://开头的链接"></el-input>
      </el-form-item>
      <!-- 投放页面 -->
      <el-form-item label="投放页面" prop="web_path">
        <el-select v-model="editFormData.web_path" @change="handleSelectChange('web_path')"
          popper-class="no-border-select">
          <el-option v-for="option in getSurpriseWebPathOptions()" :key="option.value" :label="option.label"
      ...
      <el-form-item label="广告位置" prop="position">
        <el-select v-model="editFormData.position" @change="handleSelectChange('position')"
          popper-class="no-border-select">
          <el-option v-for="option in getSurprisePositionOptions()" :key="option.value" :label="option.label"
            :value="parseInt(option.value, 10)"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="跳转地址" prop="url">
        <el-input v-model="editFormData.url" placeholder="请输入以http://或https://开头的链接"></el-input>
      </el-form-item>

      <!-- 广告开始时间 -->
      <el-form-item label="开始时间" prop="start_ts">
        <el-date-picker v-model="editFormData.start_ts" type="datetime" format="YYYY-MM-DD HH:mm:ss"
          placeholder="选择日期和时间"></el-date-picker>
      </el-form-item>
      <!-- 广告结束时间 -->
      <el-form-item label="结束时间" prop="end_ts">
        <el-date-picker v-model="editFormData.end_ts" type="datetime" format="YYYY-MM-DD HH:mm:ss" placeholder="选择日期和时间"
      ...
      <el-form-item label="支持关闭" prop="allowed_close">
        <el-switch v-model="editFormData.allowed_close" active-color="#13ce66" inactive-color="#ff4949"
          :active-value="1" :inactive-value="0"></el-switch>
      </el-form-item>
      <el-form-item label="是否显示" prop="is_show">
        <el-switch v-model="editFormData.is_show" active-color="#13ce66" inactive-color="#ff4949" :active-value="1"
