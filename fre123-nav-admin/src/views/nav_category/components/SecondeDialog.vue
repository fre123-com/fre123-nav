...
<el-dialog :model-value="visible"  @close="handleClose">
  ...
  <el-form  :model="formData" class="demo-form-inline" :rules="rules.baseRules" label-width="auto" ref="newCategoryForm_second">
      <el-form-item label="类别名称" prop="tab_name" class="form-top">
        <el-input v-model="formData.tab_name" clearable />
      </el-form-item>

      <el-form-item label="跳转链接" prop="tab_name_url">
        <el-input v-model="formData.tab_name_url" clearable />
      </el-form-item>

      <el-form-item class="close">
      <el-button v-if='editMode' type="primary" @click="onSubmit(newCategoryForm_second)" style="margin-left: 80%;">确认修改</el-button>
      <el-button v-else type="primary" @click="onSubmitNew(newCategoryForm_second)" style="margin-left: 80%;">确认新建</el-button>
      </el-form-item>
  </el-form>  
</el-dialog>
 </template>
