            ...
            function withTeleports(fileName, position) {
                ...
                for (const ref of _symbols) {
                    ...
                    if (!teleport.allowCrossFile
                        ...
                        continue;
                    for (const [teleRange] of teleport.findTeleports2(ref.textSpan.start, ref.textSpan.start + ref.textSpan.length, sideData => {
            ...
            function withTeleports(fileName, position) {
                ...
                for (const ref of _symbols.definitions) {
                    ...
                    if (!teleport.allowCrossFile
                        && sourceFiles.getSourceFileByTsUri(lsType, shared.fsPathToUri(ref.fileName)) !== sourceFiles.getSourceFileByTsUri(lsType, shared.fsPathToUri(fileName)))
                        continue;
                    for (const [teleRange] of teleport.findTeleports2(ref.textSpan.start, ref.textSpan.start + ref.textSpan.length, sideData => !!sideData.capabilities.definitions)) {
                        if (loopChecker.has(ref.fileName + ':' + teleRange.start))
                            continue;
                        withTeleports(ref.fileName, teleRange.start);
                    }
                ...
                for (const symbol of _symbols) {
                    for (const ref of symbol.references) {
                        ...
                        if (!teleport.allowCrossFile
                            && sourceFiles.getSourceFileByTsUri(lsType, shared.fsPathToUri(ref.fileName)) !== sourceFiles.getSourceFileByTsUri(lsType, shared.fsPathToUri(fileName)))
                            continue;
                        for (const [teleRange] of teleport.findTeleports2(ref.textSpan.start, ref.textSpan.start + ref.textSpan.length, sideData => !!sideData.capabilities.references)) {
                            if (loopChecker.has(ref.fileName + ':' + teleRange.start))
                                continue;
                            withTeleports(ref.fileName, teleRange.start);
                        }
