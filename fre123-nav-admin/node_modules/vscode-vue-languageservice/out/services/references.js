...
function register({ sourceFiles, getCssLs, getTsLs }) {
    ...
    function onTs(uri, position) {
        let vueResult = [];
        // vue -> ts
        for (const tsLoc of sourceFiles.toTsLocations(uri, position, position, data => !!data.capabilities.references)) {
            ...
            for (const tsLoc_2 of tsResult) {
                for (const vueLoc of sourceFiles.fromTsLocation(tsLoc.lsType, tsLoc_2.uri, tsLoc_2.range.start, tsLoc_2.range.end, data => !!data.capabilities.references)) {
                    vueResult.push({
                        uri: vueLoc.uri,
                        range: vueLoc.range,
                    });
            ...
            function withTeleports(uri, position) {
                ...
                for (const tsLoc_2 of tsLocs) {
                    ...
                    if (!teleport.allowCrossFile
                        ...
                        continue;
                    for (const [teleRange] of teleport.findTeleports(tsLoc_2.range.start, tsLoc_2.range.end, sideData => !!sideData.capabilities.references)) {
