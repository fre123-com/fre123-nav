...
function register({ sourceFiles, getTsLs }) {
    ...
    function tsTsCallHierarchyItem(item) {
        ...
        const tsItems = [];
        for (const tsLoc of sourceFiles.toTsLocations(item.uri, item.range.start, item.range.end, data => !!data.capabilities.references)) {
            if (tsLoc.type === 'source-ts' && tsLoc.lsType !== 'script')
                continue;
            for (const tsSelectionLoc of sourceFiles.toTsLocations(item.uri, item.selectionRange.start, item.selectionRange.end, data => !!data.capabilities.references)) {
                tsItems.push({
                    ...item,
                    uri: tsLoc.uri,
                    range: tsLoc.range,
