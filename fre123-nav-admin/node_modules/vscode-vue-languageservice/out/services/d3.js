...
function register({ modules: { typescript: ts }, sourceFiles, templateTsLs }) {
    ...
    return async (document) => {
        ...
        if (sourceFile) {
            ...
            for (const sourceMap of sourceFile.getTsSourceMaps()) {
                ...
                function nodeWalker(node) {
                    ...
                    if (ts.isVariableDeclaration(node)
                        ...
                        const range = (_a = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst) + node.name.getWidth(scriptAst)))) === null || _a === void 0 ? void 0 : _a[0];
                        const _argRange = getArgRanges(node.initializer, scriptAst);
                        const argsStartRange = _argRange ? (_b = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(_argRange.start), sourceMap.mappedDocument.positionAt(_argRange.start))) === null || _b === void 0 ? void 0 : _b[0] : undefined;
                        const argsEndRange = _argRange ? (_c = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(_argRange.end), sourceMap.mappedDocument.positionAt(_argRange.end))) === null || _c === void 0 ? void 0 : _c[0] : undefined;
                        ...
                        && node.name
                        && node.body) {
                        const range = (_d = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst) + node.name.getWidth(scriptAst)))) === null || _d === void 0 ? void 0 : _d[0];
                        const blockStartRange = (_e = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.body.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.body.getStart(scriptAst)))) === null || _e === void 0 ? void 0 : _e[0];
                        const blockEndRange = (_f = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.body.getStart(scriptAst) + node.body.getWidth(scriptAst)), sourceMap.mappedDocument.positionAt(node.body.getStart(scriptAst) + node.body.getWidth(scriptAst)))) === null || _f === void 0 ? void 0 : _f[0];
                        ...
                        const nameRange = (_g = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.name.getStart(scriptAst) + node.name.getWidth(scriptAst)))) === null || _g === void 0 ? void 0 : _g[0];
                        const startRange = (_h = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.initializer.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.initializer.getStart(scriptAst)))) === null || _h === void 0 ? void 0 : _h[0];
                        const endRange = (_j = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.initializer.getStart(scriptAst) + node.initializer.getWidth(scriptAst)), sourceMap.mappedDocument.positionAt(node.initializer.getStart(scriptAst) + node.initializer.getWidth(scriptAst)))) === null || _j === void 0 ? void 0 : _j[0];
                        const blockStartRange = (_k = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.initializer.body.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(node.initializer.body.getStart(scriptAst)))) === null || _k === void 0 ? void 0 : _k[0];
                        const blockEndRange = (_l = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(node.initializer.body.getStart(scriptAst) + node.initializer.body.getWidth(scriptAst)), sourceMap.mappedDocument.positionAt(node.initializer.body.getStart(scriptAst) + node.initializer.body.getWidth(scriptAst)))) === null || _l === void 0 ? void 0 : _l[0];
                        ...
                        if (name) {
                            const range = (_m = sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(name.getStart(scriptAst)), sourceMap.mappedDocument.positionAt(name.getStart(scriptAst) + name.getWidth(scriptAst)))) === null || _m === void 0 ? void 0 : _m[0];
                            const _argRange = getArgRanges(node, scriptAst);
                            const argsRange = _argRange ? sourceMap.getSourceRange(sourceMap.mappedDocument.positionAt(_argRange.start), sourceMap.mappedDocument.positionAt(_argRange.end)) : undefined;
                            if (range && argsRange) {
                                funcCalls.push({
            ...
            function nodeWalker(node, tsDoc) {
                if (ts.isVariableDeclaration(node)
                    && ts.isIdentifier(node.name)
                    && node.initializer
                    && ts.isCallExpression(node.initializer)
        ...
        const refsLinks = new Map();
        const funcsLinks = new Map();
        ...
        for (const ref of refs) {
            for (const reference of ref.references) {
                if (reference.uri === document.uri) {
                    ...
                    if (match) {
                        if (match.type === 'ref') {
                            (_a = refsLinks.get(match.name)) === null || _a === void 0 ? void 0 : _a.add(ref.name);
                        }
                        else if (match.type === 'func') {
                            (_b = funcsLinks.get(match.name)) === null || _b === void 0 ? void 0 : _b.add(ref.name);
                        }
                    }
                }
                else {
                    ...
                    (_c = fileLinks.get(fileName)) === null || _c === void 0 ? void 0 : _c.add(ref.name);
                }
            }
        }
        for (const funcCall of funcCalls) {
            for (const definition of funcCall.definitions) {
                ...
                if (uri === document.uri) {
                    const definitionFunc = funcs.find(func => range.start.line === func.range.start.line
                    ...
                    if (definitionFunc && match) {
                        if (match.type === 'ref') {
                            (_d = refsLinks.get(match.name)) === null || _d === void 0 ? void 0 : _d.add(definitionFunc.name);
                        }
                        else if (match.type === 'func') {
                            (_e = funcsLinks.get(match.name)) === null || _e === void 0 ? void 0 : _e.add(definitionFunc.name);
                        }
                    }
                }
                else {
                    ...
                    if (match) {
                        if (match.type === 'ref') {
                            (_f = refsLinks.get(match.name)) === null || _f === void 0 ? void 0 : _f.add(fileName);
                        }
                        else if (match.type === 'func') {
                            (_g = funcsLinks.get(match.name)) === null || _g === void 0 ? void 0 : _g.add(fileName);
                        }
                    }
                }
            }
        ...
        function getArgRanges(call, scriptAst) {
        ...
        function findBestMatchBlock(range) {
            const _refs = refs.filter(ref => document.offsetAt(range.start) >= document.offsetAt(ref.blockRange.start)
                && document.offsetAt(range.end) <= document.offsetAt(ref.blockRange.end));
            const _funcs = funcs.filter(func => document.offsetAt(range.start) >= document.offsetAt(func.blockRange.start)
                && document.offsetAt(range.end) <= document.offsetAt(func.blockRange.end));
            const _arr = [..._refs, ..._funcs];
            if (!_arr.length)
                return;
            return _arr.sort((a, b) => (document.offsetAt(a.blockRange.end) - document.offsetAt(a.blockRange.start))
                - (document.offsetAt(b.blockRange.end) - document.offsetAt(b.blockRange.start)))[0];
        }
    };
}
