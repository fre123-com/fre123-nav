...
function register({ sourceFiles, getCssLs, getTsLs, scriptTsLs }) {
    ...
    function onTsPrepare(uri, position) {
        let error;
        for (const tsLoc of sourceFiles.toTsLocations(uri, position, position, data => data.capabilities.rename === true || (typeof data.capabilities.rename === 'object' && data.capabilities.rename.in))) {
            ...
            for (const editUri in tsResult.changes) {
                ...
                for (const textEdit of textEdits) {
                    ...
                    if (!teleport.allowCrossFile
                        ...
                        continue;
                    for (const [teleRange, sideData] of teleport.findTeleports(textEdit.range.start, textEdit.range.end, sideData => !!sideData.capabilities.rename)) {
 ...
 */
function tsEditToVueEdit(lsType, ignoreScriptLsResult, tsResult, sourceFiles, isValidRange) {
    ...
    for (const tsUri in tsResult.changes) {
        ...
        for (const tsEdit of tsEdits) {
            for (const vueLoc of sourceFiles.fromTsLocation(lsType, tsUri, tsEdit.range.start, tsEdit.range.end, data => data.capabilities.rename === true || (typeof data.capabilities.rename === 'object' && data.capabilities.rename.out))) {
