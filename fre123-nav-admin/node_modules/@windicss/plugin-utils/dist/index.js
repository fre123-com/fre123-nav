...
async function applyExtractors(code, id, extractors = [], defaultExtract = DefaultExtractor) {
  let ext = id ? (0, import_node_path.extname)(id) : "*";
  if (ext[0] === ".")
    ext = ext.slice(1);
...
function buildGlobs(dirs, fileExtensions) {
  ...
  const globs = exts.length ? dirs.map(
    (i) => import_node_path2.default.posix.join(
      i,
...
async function resolveOptions(options = {}, utilsOptions = {}, loadConfigFile = false) {
  ...
  function resolveGlob(glob) {
    if (glob.startsWith("!"))
      return `!${(0, import_utils2.slash)(import_node_path2.default.resolve(root, glob.slice(1)))}`;
    return (0, import_utils2.slash)(import_node_path2.default.resolve(root, glob));
  }
...
function createUtils(userOptions = {}, utilsOptions = {
  ...
  const scan = (0, import_utils7.createSingletonPromise)(async () => {
    ...
    const contents = await Promise.all(
      files.filter((id) => isDetectTarget(id)).map(async (id) => [await import_node_fs.default.promises.readFile(id, "utf-8"), id])
    );
...
// src/index.ts
__reExport(src_exports, resolveOptions_exports, module.exports);
