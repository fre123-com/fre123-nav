...
function generate(sourceLang, templateAst, isVue2, cssScopedClasses = [], htmlToTemplate, isScriptSetup, searchTexts) {
    ...
    for (const tagName in tags) {
        ...
        if (!isNamespacedTag) {
            tsCodeGen.addText(`// @ts-ignore\n`);
            tsCodeGen.addText(`({ `);
            writeObjectProperty2(tagName, tagRanges, {
                vueTag: 'template',
    ...
    function writeProps(node, forRemainStyleOrClass, mode) {
        ...
        for (const prop of node.props) {
            if (prop.type === 7 /* DIRECTIVE */
                ...
                if (prop.arg && propName_1 !== propName_2) {
                    ...
                    writePropValuePrefix(isStatic);
                    tsCodeGen.addText((_f = (_e = prop.exp) === null || _e === void 0 ? void 0 : _e.loc.source) !== null && _f !== void 0 ? _f : 'undefined');
                    writePropValueSuffix(isStatic);
                    writePropEnd(isStatic);
                }
            }
        ...
        function writePropName(name, isStatic, sourceRange, data) {
            if (mode === 'props' && isStatic) {
                ...
                writeObjectProperty(name, sourceRange, data);
            }
        }
